...
describe("Game Controller", () => {
  ...
  describe("startGame", () => {
    ...
    it("should return error if starting game fails", async () => {
      (gameService.startNewGame as jest.Mock).mockRejectedValue(
  ...
  describe("getRandomLocation", () => {
    ...
    it("should return error if getting location fails", async () => {
      (gameService.getRandomLocation as jest.Mock).mockRejectedValue(
        new Error("Failed to get location")
      );
  ...
  describe("submitGuess", () => {
    it("should return score and distance when the guess is submitted successfully", async () => {
      ...
      expect(gameService.evaluateGuess).toHaveBeenCalledWith(
        "1",
        "1",
        50.0,
        50.0
    ...
    it("should return error if evaluateGuess fails", async () => {
      ...
      (gameService.evaluateGuess as jest.Mock).mockRejectedValue(
  ...
  describe("getGameResults", () => {
    ...
    it("should return error if getGameResults fails", async () => {
      ...
      (gameService.getGameResults as jest.Mock).mockRejectedValue(
        new Error(errorMessage)
      );
