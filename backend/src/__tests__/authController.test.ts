...
import { signup, login, logout, getSession } from '../controllers/authController';
...
describe('Auth Controller', () => {
  ...
  describe('signup', () => {
    it('should create a new user and return success', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'User created successfully', user });
    });

    it('should return error if signup fails', async () => {
      const req = { body: { email: 'test@example.com', password: 'password' } } as Request;

      (authService.signup as jest.Mock).mockRejectedValue(new Error('Signup failed'));
  ...
  describe('login', () => {
    it('should log in an existing user', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Login successful', session });
    });
  ...
  describe('Get Session Controller', () => {
    ...
    it('should return the session if it exists', async () => {
        ...
        (authService.getSession as jest.Mock).mockResolvedValue({ session: sessionData });
    ...
    it('should return an error if session retrieval fails', async () => {
      ...
      (authService.getSession as jest.Mock).mockRejectedValue(new Error(errorMessage));
